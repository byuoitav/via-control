...
func readPump(device structs.Device, pconn *net.TCPConn, event events.Event, viaUser string, viaPass string) {
	...
	defer func(device structs.Device) {
		...
		retryViaConnection(device, pconn, event, viaUser, viaPass)
	}(device)
